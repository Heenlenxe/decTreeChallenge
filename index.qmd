...
- Answer: Zip code should be modelled as a categorical variable.  It is a non-ordinal variable. The numbers and their ordering have no inherent relationship to house prices, at least not normally. One could argue that certain zipcodes belong to higher income neighborhoods, but this is not a guaranteed relationship and would be a very weak predictor of house prices compared to other factors like square footage, number of bedrooms, year built, etc.
...
- Answer: The R implementation of decision trees offers a more sophisticated approach to handling categorical variables. R's `rpart` package, for example, uses a more principled approach to handling categorical variables by using a decision tree algorithm that is specifically designed to handle categorical variables. This allows for more accurate and interpretable models. The website specified above specifically says "the scikit-learn implementation does not support categorical variables for now". This is a weakness in the Python implementation and R offers a more robust implementation.
